...
def diccionario_notas_desde_CSV(archivoCSV):
    with open(f"{archivoCSV}", "r", encoding="utf-8") as n:
        ...
        for fila in notas:
            if not fila or len(fila) != 3: # Si hay un salto de linea lo omitimos
                continue
...
def diccionario_becas_desde_TXT(archivoTXT):
    with open(f"{archivoTXT}", "r", encoding="utf-8") as b:
        ...
        for linea in becas:
            ...
            if len(linea) != 2: # Control de columnas
                continue

            nombre_beca = linea[0].title().strip()
            beca = linea[1].strip()
            if beca != "SI": # Control de becas en segunda fila
                beca = "NO"
            dict_becas[nombre_beca] = beca
        return dict_becas
